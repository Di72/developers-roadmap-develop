...
* Какие типы в JS изменяемые, а какие нет? Существуют три преобразованияЖ строковое, численное и булеовое + приобразование объектов в примитивы.
* Что значит ссылка на переменную? если мы меняеь объект по сылке, то мы меняем и сам объект. а вот let 2 = let 1 и мы меняем лет 2, то лет 1 сохарняется, т.к. мы меняем копию лет 1 ввиду лет 1 https://medium.com/webbdev/js-49122c7d8c26
...
* Что делает оператор `for..in`? Какие имеются особенности при использовании этого оператора с массивами? объявление переменной внутри и возможность вытаскивать значения и ключи. for...of для массива, только значения
...
splice( 1, 2) - он возвращает новый массив удаленных элементов. А первоначальный массив содержит оставшиеся элементы. У него два аргумента: первый указывает индекс элемента, который нужно удалить. второй – сколько элементов нужно удалить после указанного ключа.
...
В современном стандарте call/apply передают this «как есть». А в старом, без use strict, при указании первого аргумента null или undefined в call/apply, функция получает this = window, 
  ...
  * Как директива `use strict` влияет на `this` внутри функции? Функция начинает работать св строгом режиме
...
* Что такое замыкания, и в каких случаях они могут быть полезны?Замыкание – это функция, которая запоминает свои внешние переменные и может получить к ним доступ. Как сохранить состояние с помощью замыкания, и для чего это состояние может быть использовано?
